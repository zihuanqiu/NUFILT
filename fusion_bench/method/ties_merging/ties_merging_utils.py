...
def magmax_merging(
    ...
    """
    Perform pairwise MagMax merging between a “previous” and a “new” tensor,
    then scale by 0.5 to match the original MagMax step size.  
    flat_task_checks[0] is the previous aggregate (tv_pre),  
    flat_task_checks[1] is the new task vector (tv_new).

    Args:
        flat_task_checks (Tensor): shape (2, ...), where
            flat_task_checks[0] = tv_pre
            flat_task_checks[1] = tv_new

    Returns:
        Tensor: the scaled update delta (0.5 * element‑wise max‑abs), shape (...).
    """
    # nothing to merge
    if flat_task_checks.numel() == 0:
        return flat_task_checks

    # if only one tensor is present, just scale and return it
    if flat_task_checks.shape[0] == 1:
        return flat_task_checks[0] * 0.5

    tv_pre, tv_new = flat_task_checks[0], flat_task_checks[1]

    # 1) element‑wise pick the sign‑preserving max‑abs
    merged = torch.where(
        tv_new.abs() >= tv_pre.abs(),
        tv_new,
        tv_pre
    )
    return merged
