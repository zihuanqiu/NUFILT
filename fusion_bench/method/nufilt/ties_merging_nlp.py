...
class ContinualTiesMergingForT5(BaseAlgorithm, LightningFabricMixin):
    def __init__(
        self,
    ...
    def run(self, modelpool: BaseModelPool):
        ...
        for model_idx, model_name in tqdm(
            ...
            if model_idx == 0:
                ...
                # merged delta relative to pretrained
                merged_delta = state_dict_sub(
                    merged_model.state_dict(keep_vars=True),
                    pretrained_model.state_dict(keep_vars=True),
                )

                tv_flat = torch.vstack(
                    [
                        state_dict_to_vector(
                            merged_delta, remove_keys=self.remove_keys
                        ),
                        state_dict_to_vector(task_vector, remove_keys=self.remove_keys),
                    ]
                )
                # TIES merging across two vectors
                merged_vec = ties_merging(
                    tv_flat,
                    reset_thresh=self.threshold,
                    merge_func=self.merge_func,
                )
                # reconstruct full state-dict delta
                ties_state = vector_to_state_dict(
                    merged_vec,
                    merged_model.state_dict(),
                    remove_keys=self.remove_keys,
                )

                # Apply merged deltas to all parameters & T5LayerNorm
                for key, delta in ties_state.items():
                    # skip buffers if not param
                    if key not in merged_model.state_dict():
                        continue
                    # update weight or bias
                    orig = merged_model.state_dict()[key]
                    merged_model.state_dict()[key].copy_(
                        orig + self.scaling_factor * delta
                    )
